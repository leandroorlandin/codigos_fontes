...
arquivocommits = 'arquivocommits.json'

#ABERTURA DOS ARQUIVOS
arquivo_apps = open(arquivoapps, 'r',encoding="utf-8")
arquivo_commits = open(arquivocommits, 'w', encoding="utf-8")
...
while i < len(source_location):
    if 'https://github.com/' in source_location[i]:
        ...
        urlcommits = "https://api.github.com/repos/" + repo[19:len(repo)] + "/commits?page="
        #print(urlcommits)
        ...
        arquivocommitsespecifico = "arquivocommits" + nome_arquivo_especifico + ".json"
        arquivo_commits_especifico = open(arquivocommitsespecifico, 'w', encoding="utf-8")
        ...
        while continua == "S":
            ...
            urlcommitscompleta = urlcommits + str(page)
            numero_chamadas = numero_chamadas + 1
            numero_chamadas_total = numero_chamadas_total + 1
            print(i, " - ", numero_chamadas, "|", numero_chamadas_total, "|", "urlcompleta = ", urlcommitscompleta)
            #REQUEST NA API DE CONSULTA DO GITHUB
            try:
                requisicao_get = requests.request("GET", urlcommitscompleta, headers=headers, data = payload)
            ...
            if requisicao_get.status_code == 200 and textoget != "[]":
                ...
                arquivo_commits.write(textoget + "\n")
                arquivo_commits_especifico.write(textoget + "\n")
                page = page + 1
            else:
                continua = "N"
